...
class IndexController extends BaseController{
	...
	private function login_user($username , $password_text, $rememberme = false){
		...
		$db->where("name", $username)->orWhere("nickname", $username);
	...
	function register($formdata = null){
		if($formdata){
			...
			$db = $this->GetModel();
			$tablename = $this->tablename;
			$fields = $this->fields = array("name","nickname","password","secret_key","status"); //registration fields
			...
			$this->rules_array = array(
				'name' => 'required',
				'nickname' => 'required|valid_email',
				'password' => 'required',
				'secret_key' => 'required',
			);
			$this->sanitize_array = array(
				'name' => 'sanitize_string',
				'nickname' => 'sanitize_string',
				'secret_key' => 'sanitize_string',
			...
			//Check if Duplicate Record Already Exit In The Database
			$db->where("nickname", $modeldata['nickname']);
			if($db->has($tablename)){
				$this->view->page_error[] = $modeldata['nickname']." Already exist!";
			}
			if($this->validated()){
				$rec_id = $this->rec_id = $db->insert($tablename, $modeldata);
				if($rec_id){
					$this->login_user($modeldata['nickname'] , $password_text);
					return;
				}
				else{
					$this->set_page_error();
