...
if($_SERVER["REQUEST_METHOD"] == "POST"){
    ...
    $details=mysqli_query($dbconfig,"SELECT name,nickname FROM users WHERE nickname='$email_reg'");
    if (mysqli_num_rows($details)>0) { //if the given email is in database, ie. registered
        ...
        $to      = $email_reg;
        $subject = 'Changing password TBG livey';
        $msg = "Please copy the link and paste in your browser address bar". "\r\n"."https://tbglivery.online/user/forgot/forgot_password_reset.php?key=".$key."&email=".$email_reg;
        $headers = 'From:TBG Livery' . "\r\n";
        mail($to, $subject, $msg, $headers);
        sendEmail("Dear player",$to,$subject,$msg);
    }
    else{
        $message="Sorry! There is account associated with this email";
    }
}
?>
<!DOCTYPE html>
